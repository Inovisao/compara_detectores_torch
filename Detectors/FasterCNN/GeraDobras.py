...
def convert_coco_to_voc(fold):
    diretorio = 'dataset/Faster'
    if os.path.exists(diretorio):  
        shutil.rmtree(diretorio)
    os.makedirs(diretorio)  
    caminho_train = 'dataset/Faster/train'
    caminho_test = 'dataset/Faster/test'
    caminho_valid = 'dataset/Faster/valid'
    ...
    for Caminho in foldsUsadas:
        coco_annotation_file = 'dataset/filesJSON/'+Caminho
        path = Caminho[7:-5]
        if path == 'val':
        ...
        for image_id in tqdm(image_ids, desc="Converting images"):
            ...
            with open(os.path.join(output_dir, os.path.splitext(file_name)[0] + '.xml'), 'w') as f:
                ...
                f.write('</annotation>')
            image = 'dataset/all/train/'+file_name
