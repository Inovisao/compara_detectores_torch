...
def _resolve_split_paths(root: Path, fold: str) -> List[Tuple[str, Path, Path]]:
    """Resolve split names, annotation files and image directories for a fold."""
    files_json_dir = root / "filesJSON"
    splits: List[Tuple[str, Path, Path]] = []

    if files_json_dir.exists():
        pattern = f"{fold}_*.json"
        json_paths = sorted(p for p in files_json_dir.glob(pattern) if p.is_file())
        if not json_paths:
    ...
    # Tiled dataset structure with train/val/test folders
...
def _process_split(
    ...
    images_info = {int(info["id"]): info for info in dataset.get("images", [])}
    ...
    for image_id, image_info in images_info.items():
        ...
        if not source_image.exists():
            raise FileNotFoundError(
                f"Image '{file_name}' referenced in {json_path} not found in {images_src}"
            )
...
def CriarLabelsYOLOV5TPH(fold: str, root_dir: str | Path) -> Path:
    """Gera a estrutura de pastas e o arquivo data.yaml para o treinamento."""
    ...
    output_root = dataset_root / "YOLOV5_TPH"
    ...
    data_yaml_path = dataset_root / "data_yolov5_tph.yaml"
    _write_data_yaml(data_yaml_path, output_root, class_names, split_names)
    return data_yaml_path


__all__ = ["CriarLabelsYOLOV5TPH"]
