...
def _collect_categories(annotation_paths: List[Path]) -> Tuple[List[str], Dict[int, int]]:
    ...
    # 0-indexed: HuggingFace object detection models append the "no object"
    # class at index len(class_names) automatically, so foreground classes
    # must occupy 0..len(class_names)-1.
    mapping = {category_id: idx for idx, (category_id, _) in enumerate(sorted_items)}
    return class_names, mapping


def CriarLabelsViT(fold: str, root_dir: str | Path) -> ViTDatasetConfig:
    ...
    if "train" not in split_index or "val" not in split_index:
        raise ValueError("ViT training requires at least 'train' and 'val' splits.")
...
__all__ = ["CriarLabelsViT", "ViTDatasetConfig", "SplitPaths"]
