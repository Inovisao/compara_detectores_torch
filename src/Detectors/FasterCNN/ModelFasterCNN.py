...
paciencia = 30  # Total de épocas sem melhoria da acurácia na validação até parar
...
nome_rede = "faster"
largura_imagens = 640  # Largura das imagem para a arquitetura escolhida
altura_imagens = 640 # Altura das imagem para a arquitetura escolhida
...
path_train = os.path.join(ROOT_DATA_DIR,'faster','train')
path_val = os.path.join(ROOT_DATA_DIR,'faster','valid')
path_test = os.path.join(ROOT_DATA_DIR,'faster','test')

imagens_train = fnmatch.filter(os.listdir(path_train), "*.jpg")
imagens_val = fnmatch.filter(os.listdir(path_val), "*.jpg")
imagens_test = fnmatch.filter(os.listdir(path_test), "*.jpg")



total_imagens=len(imagens_train)+len(imagens_val)+len(imagens_test)

print(f"Total de imagens: {total_imagens}")
print(f"Total de imagens de treinamento: {len(imagens_train)} ({100*len(imagens_train)/total_imagens:>2f}%)")
print(f"Total de imagens de validação: {len(imagens_val)} ({100*len(imagens_val)/total_imagens:>2f}%)")
print(f"Total de imagens de teste: {len(imagens_test)} ({100*len(imagens_test)/total_imagens:>2f}%)")
...
treino = CustomDataset(path_train,imagens_train,largura_imagens,altura_imagens,classes,
                       A.Compose([
                                      A.Flip(0.5),  
...
val = CustomDataset(path_val,imagens_val,largura_imagens,altura_imagens,classes,
                       A.Compose([
                                    ToTensorV2(p=1.0)
                                 ], bbox_params={
                                      'format': 'pascal_voc', 
...
teste = CustomDataset(path_test,imagens_test,largura_imagens,altura_imagens,classes,
                       A.Compose([
                                    ToTensorV2(p=1.0)
                                 ], bbox_params={
                                      'format': 'pascal_voc', 
...
for epoca in range(epocas):
    ...
    if val_loss < (menor_perda-tolerancia): 
      ...
      torch.save(model.state_dict(), 'FasterCNN/'+"modelo_treinado_"+nome_rede+".pth")
