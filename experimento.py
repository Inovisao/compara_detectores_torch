...
epocas = 5  # Total de passagens durante a aprendizagem pelo conjunto de imagens
...
if nome_rede == "faster":
   ...
   model.roi_heads.box_predictor = FastRCNNPredictor(total_atributos, len(classes)) 
    ...
    # Obter a camada de previsão
    predictor_layer = model.head.classification_head.module_list[-1]
    # Obter o número total de atributos de entrada da camada de previsão
    total_atributos = predictor_layer.in_channels
    new_predictor_layer = torch.nn.Conv2d(total_atributos, len(classes)*4, kernel_size=(3, 3), padding=(1, 1))
    # classes*4 pra prever os parametros x,y,w,h da box de detecção
    #altero a camada que faz a previsão das classes
    model.head.classification_head.module_list[-1] = new_predictor_layer
    #atualizo o numero de classes no modelo
...
accuracy = Accuracy()
acc = accuracy(anotacoes_previstas,anotacoes)
